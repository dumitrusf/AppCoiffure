...
function seleccionarHora() {
    ...
    inputHora.addEventListener('input', function(e) {
        ...
        if(hora < 10 || hora > 18) {
            e.target.value = '';
            mostrarAlerta('Invalid Time', 'error', '.formulario');
        } else {
...
function mostrarResumen() {
    ...
    if(Object.values(cita).includes('') || cita.servicios.length === 0 ) {
        mostrarAlerta('Services data is missing, Date or Time', 'error', '.contenido-resumen', false);
    ...
    headingCita.textContent = 'Summary of Appointment';
    ...
    const fechaFormateada = fechaUTC.toLocaleDateString('en-UK', opciones);

    const fechaCita = document.createElement('P');
    fechaCita.innerHTML = `<span>Date:</span> ${fechaFormateada}`;

    const horaCita = document.createElement('P');
    horaCita.innerHTML = `<span>Time:</span> ${hora} Hours`;

    // Boton para Crear una cita
    const botonReservar = document.createElement('BUTTON');
    botonReservar.classList.add('boton');
    botonReservar.textContent = 'Reserve Appointment';
